...
if bandera ==1: 
    while (len (list_termi) != nro_proc):
        if (len(list_listo) <4) and (len(list_listo_susp)< 3):  # respetar multiprog
            if (len(list_nuevo)!=0): # lo saco de lista de nuevos
                if tiempo == list_nuevo[0].ta:
                    entra_a_listo = list_nuevo.pop(0) 
            else:
                entra_a_listo = list_listo.pop (0)
            #en el else deberia tratar cola de listos nomas
            entro = "no"
            Best_FIT ()
            for i in range (1,4):
                if (mem[i].proc == entra_a_listo.id): #lo busco si entro en memoria
                    entro = "si"
            
            if (entro == "si"):  #si entro a memoria, lo meto en listo tmb se ejecuta el loko
                print (f"Tiempo:{tiempo}, proceso id: {entra_a_listo.id}, entro a cola de listo")
                proceso_cargar = entra_a_listo
                ...
                while (ti_aux > 0 and quantum !=2):
                    if q == list_nuevo[0].ta: #arribo un nuevo proceso 
                        if (len(list_listo)<4) and (len(list_listo_susp)<3): #entra a listo 
                            entra_a_listo = list_nuevo.pop(0)
                            Best_FIT ()
                            if (entro == "si"):  #entra a memoria nomas el loko
                                list_listo.append(entra_a_listo)  
                                print (f"Tiempo:{q}, proceso id: {entra_a_listo.id}, entro a cola de listo")
                            else:
                                if (len(list_listo_susp)<3):
                                    list_listo_susp.append (entra_a_listo) #lo mando a list susp
                    quantum = quantum+1
                    ti_aux = ti_aux - 1
                    proceso_cargar.ti = ti_aux
                    q = q+1  #veo si se termino de ejecutar o que 
                    
                tiempo = q

                if (ti_aux==0): #el loko se termino de ejecutar libero memoria y cola de listo
                    print (f"Tiempo {q} el proceso {proceso_cargar.id} termino de ejecutarse")
                    for j in range (1,4):
                        if mem[j].proc == proceso_cargar.id: # libero la particion
                            mem[j].proc = ""
                            mem[j].bloq = 0
                            mem[j].FI = 0
                            list_termi.append (proceso_cargar)  #lo mando a terminado
                    ...
                    print (f"Al proceso {proceso_cargar.id} le queda {proceso_cargar.ti} para terminar su ejecucion")
                    list_listo.append (entra_a_listo)
                    info = []
                ...
                list_listo_susp.append (entra_a_listo)
                info = []
                for i in range (0, len(list_listo_susp)):
                    info.append(list_listo_susp[i].id)
                print (f"cola de listos suspendidos {info}")
            print ("hola")
        print (len(list_listo))
        print ("A")
        print (len(list_listo_susp))
